...
module.exports =  (options) => {
    return () => {
        let appScriptSources = combine(gulp.src([`${options.build.app}/app.js`, `!${options.build.vendor}/vendor.js`]),
        ...
        let vendorScriptSources = gulp.src(`${options.build.vendor}/vendor.js`, {read: false})
        ...
        var partialsInjectFile = gulp.src(`${options.build.templates.dir}/${options.build.templates.name}`, { read: false });
        ...
        let otherSources = gulp.src(`${options.build.styles}/main.css`, {read: false});
